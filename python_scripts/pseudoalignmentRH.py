...
#customises this to a new folder with parameterising names
    #using the r protects the backslashes
    #. means any character + means 1 or more times, () mean save for later, put it in a variable 
    #(.+) replaces with whatever is before 1 or 2 in the _fastq.gz file 
#need to sort out the output_dir - says find this bit, and replace with an empty string
@#-i gives you the reference genome created from 
...
#tells you how many reads it pseduo aligned and how many it cant, basically a QC step how well
#the pseudo alignment worked
